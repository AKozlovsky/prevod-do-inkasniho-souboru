...
class converter 
{
	...
	public function addAccountFile($type)
	{
		$item = new converter_account_file($type);
		$this->items[] = $item;
		return $item;
	}
	...
	{
		$res = sprintf("%s%09d%s%-30s%010d%-294s\r\n", self::HEADER, 720, $this->date, $this->filler, 109100179, $this->filler);
...
class converter_account_file
{
	...
	public function __construct($type = self::INKASO){
		$this->type = $type;
		$this->setDate();
	}
	...
	public function generate()
	{
		...
		$res .= sprintf("TI%09d%s%06d%018d%-177s%05d%-128s\r\n", 720, $this->date, count($this->items), $this->getAmount(), $this->filler, $this->filler, $this->filler);
		return $res;
	}

	public function addItem()
	{
		$item = new converter_item($this->type);
		$this->items[] = $item;
		return $item;
	}
...
class converter_item 
{	
	private $bankCode = "0100";
	private $type;
	private $pre_account_number = 0;
	private $account_number = 0;
	private $pre_destAccount_number = 0;
	private $destAccount_number = 0;
	private $amount;
	private $const_sym = 0;
	private $variable_sym = 0;
	private $spec_sym = null;
	private $srcBank_code = 0;
	private $destBank_code = 0;
	private $items = array();
	private $dueDate;
	private $date_expose;	
	private $currency;
	...
	public function setDateExpose($date = null)
	{
		if($date == null) {
			date_default_timezone_set('Europe/Prague');
			$date = date('Ymd');
		}
		$this->date_expose = $date;
		return $this;
	}
	
	public function setBankCode($bankCode)
	{
		$this->destBank_code = $bankCode;
		return $this;
	}
	...
	public function setAccount($account)
	{
		if (strpos($account, '-') !== false) {
			...
			$pre_account = $str[0];
			$account = $str[1];
			$this->pre_account_number = $pre_account;
	...
	public function setDestAccount($account)
	{
		if (strpos($account, '-') !== false) {
			...
			$pre_account = $str[0];
			$account = $str[1];
			$this->pre_destAccount_number = $pre_account;
			$this->destAccount_number = $account;
		} else {
			$this->destAccount_number = $account;
		}
		return $this;
	...
	{
		$res = "";
		$res .= sprintf("01%05d%s%s%s%015d%s%04d%010d", $this->number, $this->date_expose, $this->dueDate, $this->currency, $this->getAmount(), $this->type, $this->filler, $this->const_sym);
		$res .= sprintf("%-143s", $this->filler);
		$res .= sprintf("%04d%06d%010d%010d%010d", "0100", $this->pre_account_number ,$this->account_number, $this->variable_sym, $this->spec_sym);
		$res .= sprintf("%-33s", $this->filler);
		$res .= sprintf("%04d%06d%010d%010d%010d", $this->destBank_code, $this->pre_destAccount_number, $this->destAccount_number, $this->variable_sym, $this->spec_sym);
		$res .= sprintf("%-39s", $this->filler);
		$res .= "\r\n";
		return $res;
	}
}
...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    */
    ...
    private $varsym_col;
    private $constsym_col;
    private $specsym_col;
    private $date_col;
    private $amount_col;
    private $destAccount_col;
    private $bankCode_col;
    private $toogle_input;
    ...
    */
	...
	public function rangeToArray($pRange = 'A1', $nullValue = null, $calculateFormulas = true, $formatData = true, $returnCellRef = false) {
        ...
        // Create file format with extension .ikm
        $converter = new converter();
        $account = $converter->addAccountFile(converter_account_file::INKASO);
        
        $items_varSym = array();
        $items_specSym = array();
        $items_constSym = array();
        $items_date = array();
        $items_amount = array();
        $items_destAccount = array();
        $items_bankCode = array();
                    ...
                    if ($cell->getValue() !== null) {
                    	if ($cell->getValue() instanceof PHPExcel_RichText) {
                    		...
                    		if ($calculateFormulas) {
                    			...
                    			if ($rRef == '1') {
	                    			if ($returnValue[$rRef][$cRef] == iconv("Windows-1250", "UTF-8", 'Varsym')) {
	                    				$this->varsym_col = $cRef;
	                    			}
	                    			if ($returnValue[$rRef][$cRef] == iconv("Windows-1250", "UTF-8", 'Konstsym')) {
	                    				$this->constsym_col = $cRef;
	                    			}
	                    			if ($returnValue[$rRef][$cRef] == iconv("Windows-1250", "UTF-8", 'Specsym')) {
	                    				$this->specsym_col = $cRef;
	                    			} 
	                    			if ($returnValue[$rRef][$cRef] == iconv("Windows-1250", "UTF-8", 'Splatno')) {
	                    				$this->date_col = $cRef;
	                    			}
	                    			if ($returnValue[$rRef][$cRef] == iconv("Windows-1250", "UTF-8", 'K likvidaci')) {
	                    				$this->amount_col = $cRef; 
	                    			}
	                    			if ($returnValue[$rRef][$cRef] == iconv("Windows-1250", "UTF-8", 'Èíslo úètu')) {
	                    				$this->destAccount_col = $cRef; 
	                    			}			
	                    			if ($returnValue[$rRef][$cRef] == iconv("Windows-1250", "UTF-8", 'Kód banky')) {
	                    				$this->bankCode_col = $cRef;
	                    			}
                    			}
                    			
                    			if ($rRef !== '1') {			
                    				if ($cRef == $this->varsym_col) {
                    					$returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
                    					$items_varSym[] = $returnValue[$rRef][$cRef];
                    				} else if ($cRef == $this->constsym_col) {
                    					$returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
                    					$items_constSym[] = $returnValue[$rRef][$cRef];
                    				} else if ($cRef == $this->specsym_col) {
                    					$returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
                    					$items_specSym[] = $returnValue[$rRef][$cRef];
                    				} else if ($cRef == $this->date_col) {
                    					$returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
                    					$date = date('Ymd', PHPExcel_Shared_Date::ExcelToPHP($returnValue[$rRef][$cRef]));
                    					$items_date[] = $date;
                    				} else if ($cRef == $this->amount_col) {
                    					$returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
                    					$items_amount[] = $returnValue[$rRef][$cRef];
                    				} else if ($cRef == $this->destAccount_col) {
                    					$returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
                    					$items_destAccount[] = $returnValue[$rRef][$cRef];
                    				} else if ($cRef == $this->bankCode_col) {
                    					$returnValue[$rRef][$cRef] = $cell->getCalculatedValue();
                    					$items_bankCode[] = $returnValue[$rRef][$cRef];
                    				}
                    			} else {
                    				$returnValue[$rRef][$cRef] = null;
                    			}                    			
                    		} else {
						...
						if ($formatData) {
	                        ...
	                        $returnValue[$rRef][$cRef], ($style && $style->getNumberFormat()) ? $style->getNumberFormat()->getFormatCode() :
							PHPExcel_Style_NumberFormat::FORMAT_GENERAL);
						}
					} else {
                		// Cell holds a NULL
						if ($cRef == $this->varsym_col) {
                    		$returnValue[$rRef][$cRef] = 0;
                    		$items_varSym[] = $returnValue[$rRef][$cRef];
                    	} else if ($cRef == $this->constsym_col) {
                    		$returnValue[$rRef][$cRef] = 0;
                    		$items_constSym[] = $returnValue[$rRef][$cRef];
        ...
        if ($this->toogle_input == 1)
        {
	        $file = fopen($fileMemory, "a+");
	        if ($file !== null)
	        {
	        	$content = file_get_contents($fileMemory);
	        	$parts = explode("/", $content);
	        	
	        	if (count($parts) > 1)
	        	{
	        		$reversedParts = explode('/', strrev($content), 1);
	        		$string = strrev($reversedParts[0]);	        		
	        		$last = substr("$string", (strrpos($string, "/") + 1));
	        		$minRows = $last;
	        		$maxRows = $last + $maxRow - 1;
	        		if (strlen($maxRows) > 5)
	        		{
	        			$minRows = 0;
	        		}
        ...
        for ($i = 0; $i < $maxRow - 1; $i++) {
        	$item = $account->addItem();
        	if ($this->toogle_input == 1) {
        		$item->setNumber(++$minRows);
        	} else {
        		$item->setNumber($i + 1);
        	}
        	$item->setDateExpose($this->date_expose);
        	$item->setDate($this->dueDate);
        	$item->setCurrency('CZK');
        	if ($this->amount_col != null) $item->setAmount($items_amount[$i]);
        	if ($this->varsym_col != null) $item->setVarSym($items_varSym[$i]);
        	if ($this->constsym_col != null) $item->setConstSym($items_constSym[$i]);
        	if ($this->specsym_col != null) $item->setSpecSym($items_specSym[$i]);
        	$item->setAccount($this->account_number);
        	if ($this->destAccount_col != null) $item->setDestAccount($items_destAccount[$i]);
        	if ($this->bankCode_col != null) $item->setBankCode($items_bankCode[$i]);
        }
        
        $this->output = $converter->generate();
        date_default_timezone_set('Europe/Prague');
        $date = date('dmy');
        $fileName = 'vypisy_' . $date . '.ikm';
...
/**
	...
	*/
	...
	public function getAccountFromInput($account){
		if (strpos($account, '/') !== false) {
			$str = explode('/', $account);
			$this->account_number = $str[0];
		} else {
			$this->account_number = $account;
		}	
		return $this;
